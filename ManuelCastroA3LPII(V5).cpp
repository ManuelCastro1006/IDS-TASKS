...
int main() {
	...
	if (ret == SQL_SUCCESS || ret == SQL_SUCCESS_WITH_INFO) {
		...
		wcout << L"+-------+----------------------------+---------------------+---------------+-------------------+---------------------+------------+" << endl;
		wcout << L"| ID:   | Nombre completo            + Fecha_de_nacimiento | RFC           | Centro de trabajo | Puesto              | Directivo  |" << endl;
		wcout << L"+-------+----------------------------+---------------------+---------------+-------------------+---------------------+------------+" << endl;
		...
		if (ret == SQL_SUCCESS || ret == SQL_SUCCESS_WITH_INFO) {
			int ID_empleado;
			SQLWCHAR Nombre[50], Apellido_paterno[50], Apellido_materno[50], Fecha_de_nacimiento[50], RFC[50],
				Centro_de_trabajo[50], Puesto[50], Directivo[50];
			...
			while (SQLFetch(hStmt) == SQL_SUCCESS) {
				...
				SQLGetData(hStmt, 7, SQL_C_WCHAR, Centro_de_trabajo, sizeof(Centro_de_trabajo), NULL);
				...
				wcout << L"| " << setw(6) << left << ID_empleado
					  ...
					  << L"| " << setw(14) << left << RFC
					  << L"| " << setw(18) << left << Centro_de_trabajo
					  << L"| " << setw(20) << left << Puesto
